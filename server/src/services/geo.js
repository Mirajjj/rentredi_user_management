...
/**
 * Derive geo fields from a US zip code using the OpenWeather current-weather
 * endpoint. We use this endpoint (rather than the geo/zip lookup) because it
 * returns `coord` *and* `timezone` in a single call. `timezone` is a UTC offset
 * in seconds, stored verbatim per the assignment.
 *
 * @param {string} zipCode - 5-digit US zip (already validated upstream).
 * @returns {Promise<import("../schemas/user.js").Geo>}
 */
export async function geocodeZip(zipCode) {
  ...
  const { coord, timezone } = openWeatherResponseSchema.parse(data);
